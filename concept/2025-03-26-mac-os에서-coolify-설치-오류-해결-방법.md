...
Coolify는 Heroku나 Railway 같은 PaaS(서비스형 플랫폼)를 자기 서버에 직접 올려 쓸 수 있게 해주는 오픈소스 도구다. 깃 저장소를 연결하면 코드를 자동으로 빌드하고 배포해준다. 주 타겟은 리눅스 서버인데, 개발자가 로컬 맥에서 먼저 테스트해보려다 오류를 만나는 경우가 많다.
...
| Docker Desktop | Mac/Windows에서 리눅스 컨테이너를 실행할 수 있게 해주는 앱 |
