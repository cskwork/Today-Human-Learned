+++
title = "LinkedHashMap을 구분자로 이어 붙인 문자열로 변환하기"
date = "2024-06-04"
description = "`LinkedHashMap`의 키-값 쌍을 삽입 순서 그대로 유지하면서 원하는 구분자로 연결된 문자열로 만들려면 `StringBuilder`로 직접 순회하거나 `String.join` 계열 API를 쓴다."
tags = ["java"]
categories = ["java"]
draft = false
ShowToc = true
