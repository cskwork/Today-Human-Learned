+++
title = "Hibernate 네이밍 전략 충돌 해결"
date = "2024-06-24"
description = "`PrefixPhysicalNamingStrategy`와 `PrefixQueryModifier`를 함께 쓰면 테이블 이름이 이중으로 변조되어 쿼리가 깨진다. 네이밍 전략은 하나만 선택하고, 쿼리 가로채기는 표준 JPA 기능으로 대체하라."
tags = ["java"]
categories = ["java"]
draft = false
ShowToc = true
