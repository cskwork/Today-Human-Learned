+++
title = "Java에서 Jedis로 Redis 연동하기 — 실시간 메시징의 기반"
date = "2024-06-22"
description = "Jedis는 Java에서 Redis를 쓰는 클라이언트 라이브러리다. Pub/Sub 패턴과 큐를 조합하면 실시간 메시징 시스템을 빠르게 구축할 수 있다."
tags = ["java"]
categories = ["java"]
draft = false
ShowToc = true
...
Jedis는 이 Redis를 Java 코드에서 직접 호출하는 클라이언트다.
