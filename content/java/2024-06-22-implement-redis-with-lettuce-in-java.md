+++
title = "Lettuce로 Redis 연동하기 — Java 비동기 클라이언트 입문"
date = "2024-06-22"
description = "Lettuce는 스레드를 막지 않고 Redis와 통신하는 Java 클라이언트다. 요청을 보내고 결과를 기다리는 동안 다른 일을 할 수 있어, 대규모 실시간 시스템에서 기존 Jedis보다 처리량이 높다."
tags = ["java"]
categories = ["java"]
draft = false
ShowToc = true
