+++
title = "Git이 새 브랜치를 부모에 자동으로 연결시키는 함정 — `branch.autoSetupMerge`"
date = "2026-05-19"
description = "`git checkout -b feature/x origin/main` 하면 git이 기본적으로 `feature/x`의 upstream을 `origin/main`으로 박는다. 이후 무심코 `git push` 하면 `main`을 향해 푸시가 날아갈 수 있다. 해결책 한 줄: `git config --global branch.autoSetupMerge simple`"
tags = ["git"]
categories = ["git"]
draft = false
ShowToc = true
