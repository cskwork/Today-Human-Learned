+++
title = "ReAct 에이전트 프레임워크 — 어떤 걸 골라야 할까"
date = "2025-06-01"
description = "ReAct 패턴을 지원하는 프레임워크는 여럿이지만, 성숙도와 생산 준비 여부가 제각각이다. 시작점은 LangChain, 복잡한 흐름 제어가 필요하면 LangGraph, 다중 에이전트 협업이 목적이면 CrewAI를 선택하라."
tags = ["ai"]
categories = ["ai"]
draft = false
ShowToc = true
