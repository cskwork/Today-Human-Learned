...
date = "2024-06-06"
description = "Node.js는 JavaScript를 서버에서 실행하는 런타임으로, 비동기 I/O 덕분에 동시 요청을 적은 자원으로 처리한다."
tags = ["concept"]
categories = ["concept"]
draft = false
ShowToc = true
...
> **TL;DR**
> Node.js는 JavaScript를 서버에서 실행하는 런타임으로, 비동기 I/O 덕분에 동시 요청을 적은 자원으로 처리한다.

---

## Node.js를 왜 쓰는지 감 잡기

원래 JavaScript는 브라우저 안에서만 동작했다. 웹 페이지의 버튼 클릭을 처리하고 화면을 바꾸는 것이 전부였다. 2009년 Node.js가 등장하면서 JavaScript를 서버에서도 실행할 수 있게 됐다. 덕분에 프론트엔드와 백엔드를 같은 언어로 작성할 수 있고, 채팅이나 실시간 알림처럼 동시 연결이 많은 서비스에 특히 강하다.
