...
date = "2024-05-26"
description = "Bootstrap은 그리드 시스템, 미리 만들어진 UI 컴포넌트, 유틸리티 클래스를 제공해 CSS를 거의 안 짜도 반응형 웹을 만들 수 있는 프론트엔드 프레임워크다."
tags = ["concept"]
categories = ["concept"]
draft = false
ShowToc = true
...
> **TL;DR**
> Bootstrap은 그리드 시스템, 미리 만들어진 UI 컴포넌트, 유틸리티 클래스를 제공해 CSS를 거의 안 짜도 반응형 웹을 만들 수 있는 프론트엔드 프레임워크다.

---

## Bootstrap을 왜 쓰는지 감 잡기

웹 페이지를 처음부터 만들면 버튼 하나, 네비게이션 바 하나에도 CSS를 수십 줄 써야 한다. 모바일 화면 대응까지 고려하면 코드량은 더 늘어난다. Bootstrap은 이런 반복 작업을 HTML 클래스 이름 몇 개로 해결한다. 전 세계에서 가장 널리 쓰이는 CSS 프레임워크 중 하나로, CDN 링크 한 줄로 바로 시작할 수 있다.
