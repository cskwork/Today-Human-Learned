+++
title = "Vue.js 입문 — 왜 쓰고 어떻게 돌아가는가"
date = "2024-05-26"
description = "Vue.js는 데이터가 바뀌면 화면이 자동으로 따라 바뀌는 구조(반응형)를 컴포넌트 단위로 조립하게 해주는 프레임워크다."
tags = ["javascript"]
...
HTML과 JavaScript를 직접 연결하면, 데이터가 바뀔 때마다 DOM을 수동으로 찾아서 갱신해야 한다. 화면이 복잡해질수록 코드가 뒤엉킨다. Vue.js는 데이터와 화면을 자동으로 동기화하고, UI를 독립적인 컴포넌트로 쪼개 관리하게 해준다. 단일 페이지 애플리케이션(SPA), 관리자 대시보드, 폼 중심 서비스에 널리 쓰인다.
