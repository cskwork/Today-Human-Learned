+++
title = "Vue.js 생명주기 — 컴포넌트가 만들어지고 사라지는 과정"
date = "2024-06-10"
description = "Vue 컴포넌트는 생성 → 마운트 → 업데이트 → 소멸 네 단계를 거치며, 각 단계 전후에 훅 함수를 등록해 원하는 작업을 실행할 수 있다."
tags = ["javascript"]
...
컴포넌트가 화면에 나타나는 순간 서버에서 데이터를 불러오거나, 제거되는 순간 타이머를 멈춰야 하는 상황이 자주 있다. Vue는 이런 "특정 시점에 코드를 실행"하는 기능을 생명주기 훅으로 제공한다. 훅을 모르면 데이터를 너무 일찍 불러오거나(DOM이 아직 없는 상태), 정리를 깜빡해 메모리 누수 버그를 만든다.
