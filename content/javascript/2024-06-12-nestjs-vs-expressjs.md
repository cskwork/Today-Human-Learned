+++
title = "NestJS vs ExpressJS — 언제 무엇을 쓸까"
date = "2024-06-12"
description = "ExpressJS는 자유롭지만 구조를 직접 설계해야 하고, NestJS는 구조가 정해져 있어 팀 프로젝트나 대형 앱에 적합하다."
tags = ["javascript"]
...
Node.js로 서버를 만들 때 가장 먼저 만나는 선택이 바로 이 둘이다. ExpressJS는 Node.js에서 가장 오래된 웹 프레임워크로, 라우팅과 미들웨어만 제공하고 나머지는 개발자가 직접 결정한다. NestJS는 ExpressJS 위에서 동작하지만 모듈, 의존성 주입, TypeScript 지원 같은 구조를 미리 갖추고 있다.
