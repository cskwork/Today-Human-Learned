+++
title = "Express.js 미들웨어 — 요청이 응답이 되기까지 거치는 관문"
date = "2024-06-11"
description = "미들웨어는 요청(req)과 응답(res) 사이에서 실행되는 함수이고, `next()`를 호출해야 다음 단계로 넘어간다."
tags = ["javascript"]
...
클라이언트 요청이 서버에 도달해도 바로 비즈니스 로직으로 가지 않는다. 그 전에 처리해야 할 일이 많다. 요청 본문을 파싱하고, 로그를 남기고, 사용자가 로그인했는지 확인하고, 에러가 생기면 잡아서 응답한다.
