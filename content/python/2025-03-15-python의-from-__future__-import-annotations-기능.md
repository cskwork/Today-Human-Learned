+++
title = "Python의 `from __future__ import annotations` — 타입 힌트를 나중에 평가하기"
date = "2025-03-15"
description = "이 한 줄을 파일 맨 위에 쓰면 타입 힌트를 즉시 실행하지 않고 문자열로 저장해 둔다. 자기 자신을 참조하거나 두 모듈이 서로를 import할 때 발생하는 오류를 막는다."
tags = ["python"]
categories = ["python"]
draft = false
ShowToc = true
