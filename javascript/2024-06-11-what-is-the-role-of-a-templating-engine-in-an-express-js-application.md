# Express.js 템플릿 엔진 — 서버가 HTML을 만드는 방식
...
HTML 파일을 사용자마다 따로 만들 수는 없다. 회원 이름, 주문 목록, 날짜 같은 내용이 사람마다 다르기 때문이다. 템플릿 엔진은 이 문제를 해결한다. HTML에 빈칸(플레이스홀더)을 만들어 두고, 요청이 들어올 때 서버가 실제 데이터를 채워 넣어 완성된 HTML을 보낸다.
