...
컴포넌트가 화면에 나타날 때 API를 호출하고, 데이터가 바뀔 때 추가 처리를 하고, 화면에서 사라질 때 타이머를 정리해야 할 때가 있다. 이런 작업들을 "언제" 실행할지 알려면 컴포넌트의 생명주기를 이해해야 한다. 생명주기를 무시하면 메모리 누수나 API 중복 호출 같은 문제가 생긴다.
