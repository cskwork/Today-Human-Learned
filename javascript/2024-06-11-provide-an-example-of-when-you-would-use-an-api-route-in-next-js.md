# Next.js API 라우트 — 백엔드 서버 없이 서버 로직 처리하기
...
React 컴포넌트에서 외부 API를 직접 호출하면 API 키가 브라우저에 노출된다. 별도 Express 서버를 운영하기에는 규모가 작다. API 라우트는 이 사이 어딘가에 있는 해법이다. Next.js 프로젝트 안에 파일 하나를 추가하면 그게 곧 서버 엔드포인트가 된다.
